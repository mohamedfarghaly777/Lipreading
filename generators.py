...
from list import get_list_safe
...
class BasicGenerator(keras.callbacks.Callback):
    def __init__(self, dataset_path, minibatch_size, img_c, img_w, img_h, frames_n, absolute_max_string_len=30, **kwargs):
        ...
        self.face_predictor_path = kwargs.get('face_predictor_path', "/content/drive/My Drive/LipNet/common/predictors/shape_predictor_68_face_landmarks.dat")
    ...
    def build(self, **kwargs):
        ...
        print("Dataset Time is " + str(end - start))
    ...
    def enumerate_videos(self, path):
        video_list = []
        print(self.vtype)
        print(self.face_predictor_path) 
    ...
    def enumerate_align_hash(self, video_list):
        ...
        for video_path in video_list:
            ...
            print(os.path.splitext(video_path)[0].split('/'))
            print(os.path.splitext(video_path)[0].split('/')[-1])
    ...
    def next_train(self):
        ...
        while 1:
            # prin( "SI: {}, SE: {}".format(self.cur_train_index.value, self.shared_train_epoch.value))
            ...
            if epoch_differences > 0:
                ...
                # prin( "GENERATOR EPOCH {}".format(self.process_train_epoch))
                # rin( self.train_list[0])
            # prin "PI: {}, SI: {}, SE: {}".format(cur_train_index, self.cur_train_index.value, self.shared_train_epoch.value))
            ...
            # prin( "Train [{},{}] {}:{}".format(self.process_train_epoch, epoch_differences, cur_train_index,cur_train_index+self.minibatch_size))
            ret = self.get_batch(cur_train_index, self.minibatch_size, train=True)
            # if epoch_differences > 0:
            #     rint( "GENERATOR EPOCH {} - {}:{}".format(self.process_train_epoch, cur_train_index, cur_train_index + self.minibatch_size))
            #     int( ret[0]['source_str'])
            #     int( "-------------------")
            yield ret
    ...
    def next_val(self):
        while 1:
            ...
            # prin( "Val [{}] {}:{}".format(self.process_epoch, cur_val_index,cur_val_index+self.minibatch_size))
...
# daasets/video/<sid>/<id>/<image>.png
# or datasets/[train|val]/<sid>/<id>.mpg
# datasets/align/<id>.align
