...
def train(run_name, speaker, start_epoch, stop_epoch, img_c, img_w, img_h, frames_n, absolute_max_string_len, minibatch_size):
    ...
    lip_gen = BasicGenerator(dataset_path=DATASET_DIR,minibatch_size=minibatch_size,img_c=img_c, img_w=img_w, img_h=img_h, frames_n=frames_n,absolute_max_string_len=absolute_max_string_len,curriculum=curriculum, start_epoch=start_epoch).build()

    lipnet = LipNet(img_c=img_c, img_w=img_w, img_h=img_h, frames_n=frames_n,absolute_max_string_len=absolute_max_string_len, output_size=lip_gen.get_output_size())
    lipnet.summary()
    ...
    #weight_file = os.path.join(OUTPUT_DIR, "overlapped-weight.h5")
...
if __name__ == '__main__':
    ...
    run_name = "TrainModel2.0"
    speaker = "s1"
    start = time.time()
    train(run_name, speaker, 982, 10000, 3, 100, 50, 75, 32, 8)
    end = time.time()
